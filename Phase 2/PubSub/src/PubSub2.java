...
public class PubSub2 {
	...
	public static void main(String[] args) {	
		...
		while(loop!=0) {
			...
			loop = s3.nextInt();
			System.out.println("Subscribers list is ready.");	
		...
		try{  
	           ...
	           String[] remote_pub = str.split(",", 0);
	           System.out.println("topic= "+remote_pub[1]);
	           ArrayList<Integer> subscribers = Matcher.match(remote_pub[1]);
	           System.out.println(subscribers);
				if(subscribers!=null) {
					Notifier.notify(subscribers,remote_pub[2],Integer.parseInt(remote_pub[0]));
				}
	           ss.close();  
